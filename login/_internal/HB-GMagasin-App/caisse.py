...
class Caisse:
    def __init__(self, root):
        ...
        title = Label(self.root, text="Caisse Magasin ", image=self.icon_title, font=("times new roman",30,"bold"), bg="cyan", fg="black", anchor="w", padx=10, compound=LEFT).place(x=0, y=0, relwidth=1, height=80)

        #Bouton deconnecter
        btn_deconnecter = Button(self.root, command=self.deconnecter, text="Deconnexion", cursor="hand2", font=("times new roman",20,"bold"), bg="orange").place(x=1090, y=10)
        ...
        self.heure.place(x=0, y=70, relwidth=1, height=30)
        self.modif_heure()
        ...
        produit_frame1.place(x=0, y=100, width=450, height=810)
        ...
        produit_frame2.place(x=5, y=40, width=500, height=130)
        
        #Rechercher Fournisseur
        recherche = Label(produit_frame2, text="Rechercher Produit | par Nom", font=("goudy old style", 20, "bold"), bg="green", fg="white", bd=3, relief=RIDGE).place(x=2, y=10)
        nomP = Label(produit_frame2, text="Nom", font=("goudy old style", 20, "bold"), bg="white").place(x=2, y=70)
        ecri_nomP = Entry(produit_frame2, textvariable=self.var_nomP, font=("goudy old style", 20, "bold"), bg="lightyellow").place(x=70, y=70, width=140)
        ...
        btn_recherche = Button(produit_frame2, command=self.recherche, text="Recherche", font=("times new roman",15, ""), cursor="hand2", bg="green",fg="white").place(x=230, y=70, width=130, height=35)
        tous = Button(produit_frame2, command=self.affiche_resultat, text="Tous", font=("times new roman",15, ""), cursor="hand2", bg="lightgray",fg="black").place(x=370, y=70, width=50, height=35)

        #Liste Produit
        produit_frame3 = Frame(produit_frame1, bd=3, relief=RIDGE)
        produit_frame3.place(x=2, y=170, relwidth=1, height=230)
        ...
        client_frame.place(x=455, y=100, width=550, height=90)
        ...
        ecri_cli_nom = Entry(client_frame, textvariable=self.var_cli_nom, font=("goudy old style",15), bg="lightyellow").place(x=75, y=40, width=160)

        cli_contact = Label(client_frame, text="Contact", font=("goudy old style",15), bg="white").place(x=260, y=40)
        ecri_cli_contact = Entry(client_frame, textvariable=self.var_cli_contact, font=("goudy old style",15), bg="lightyellow").place(x=350, y=40, width=160)
        
        #Calculatrice
        self.var_calcul = StringVar()

        calcul_cart_frame = Frame(self.root, bd=4, relief=RIDGE, bg="white")
        calcul_cart_frame.place(x=455, y=200, width=550, height=430)

        calcul_frame = Frame(calcul_cart_frame, bd=4, relief=RIDGE, bg="white")
        calcul_frame.place(x=10, y=10, width=243, height=370)
        ...
        #Boutons
        self.btn_7 = Button(calcul_frame, text="7", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(7)).grid(row=1, column=0)
        self.btn_8 = Button(calcul_frame, text="8", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(8)).grid(row=1, column=1)
        self.btn_9 = Button(calcul_frame, text="9", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(9)).grid(row=1, column=2)
        self.btn_plus = Button(calcul_frame, text="+", font=("arial",10, "bold"), bg="skyblue", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal("+")).grid(row=1, column=3)

        self.btn_4 = Button(calcul_frame, text="4", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(4)).grid(row=2, column=0)
        self.btn_5 = Button(calcul_frame, text="5", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(5)).grid(row=2, column=1)
        self.btn_6 = Button(calcul_frame, text="6", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(6)).grid(row=2, column=2)
        self.btn_mois = Button(calcul_frame, text="-", font=("arial",10, "bold"), bg="skyblue", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal("-")).grid(row=2, column=3)

        self.btn_1 = Button(calcul_frame, text="1", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(1)).grid(row=3, column=0)
        self.btn_2 = Button(calcul_frame, text="2", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(2)).grid(row=3, column=1)
        self.btn_3 = Button(calcul_frame, text="3", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(3)).grid(row=3, column=2)
        self.btn_fois = Button(calcul_frame, text="*", font=("arial",10, "bold"), bg="skyblue", cursor="hand2", width=6,  pady=30, command=lambda:self.get_input_cal("*")).grid(row=3, column=3)

        self.btn_0 = Button(calcul_frame, text="0", font=("arial",10, "bold"), bg="gray", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal(0)).grid(row=4, column=0)
        self.btn_clear = Button(calcul_frame, text="C", font=("arial",10, "bold"), bg="pink", cursor="hand2", width=6,  pady=25, command=self.clear_cal).grid(row=4, column=1)
        self.btn_egal = Button(calcul_frame, text="=", font=("arial",10, "bold"), bg="lightgreen", cursor="hand2", width=6,  pady=25, command=self.resultat).grid(row=4, column=2)
        self.btn_div = Button(calcul_frame, text="/", font=("arial",10, "bold"), bg="skyblue", cursor="hand2", width=6,  pady=25, command=lambda:self.get_input_cal("/")).grid(row=4, column=3)


        panier_cart_frame = Frame(calcul_cart_frame, bd=3, relief=RIDGE)
        panier_cart_frame.place(x=250, y=10, width=300, height=300)
        ...
        btn_frame.place(x=0, y=500, width=450, height=130)

        Prod_nom = Label(btn_frame, text="Nom Produit", font=("goudy old style",15), bg="white").place(x=5, y=5)
        ecri_Prod_nom = Entry(btn_frame, textvariable=self.var_nomProduit, font=("goudy old style",17), bg="lightyellow", state="readonly").place(x=5, y=40, width=130, height=30)

        prix = Label(btn_frame, text="Prix Produit", font=("goudy old style",15), bg="white").place(x=155, y=5)
        ecri_prix = Entry(btn_frame, textvariable=self.var_prix, font=("goudy old style",20), bg="lightyellow", state="readonly").place(x=155, y=40, width=130, height=30)
        
        quantite = Label(btn_frame, text="Quantite Produit", font=("goudy old style",15), bg="white").place(x=280, y=5)
        quantite = Entry(btn_frame, textvariable=self.var_quantite, font=("goudy old style",20), bg="lightyellow").place(x=310, y=40, width=100, height=30)
        
        self.stock = Label(btn_frame, text="En Stock", font=("goudy old style",15), bg="white")
        self.stock.place(x=5, y=80)

        btn_clear_cart = Button(btn_frame, command=self.clear_cart, text="Clear", font=("times new roman",20), cursor="hand2", bg="lightgray",fg="black").place(x=110, y=80, width=100, height=40)
        btn_miseajour = Button(btn_frame, command=self.ajout_modifier, text="Ajouter/Modifie", font=("times new roman",20), cursor="hand2", bg="orange",fg="black").place(x=230, y=80, height=40)
        
        #Facture
        facture_frame = Frame(self.root, bd=4, relief=RIDGE, bg="white")
        facture_frame.place(x=1000, y=100, width=270, height=400)

        f_title = Label(facture_frame, text="Zone de Facture", font=("goudy old style",20,"bold"), bg="lightblue", bd=3, relief=RIDGE).pack(side=TOP, fill=X)
        ...
        fact_Menu_frame.place(x=750, y=520, width=520, height=110)

        self.montant = Label(fact_Menu_frame, text="Montant Facture \n [0]", font=("goudy old style",15,"bold"), bg="#3f51b5", fg="white")
        self.montant.place(x=30, y=5, width=160, height=50)

        self.remise = Label(fact_Menu_frame, text="Remise \n [0]", font=("goudy old style",15,"bold"), bg="#8bc34a", fg="white")
        self.remise.place(x=200, y=5, width=90, height=50)

        self.net_payer = Label(fact_Menu_frame, text="Net a Payer \n [0]", font=("goudy old style",15,"bold"), bg="#607d8b", fg="white")
        self.net_payer.place(x=300, y=5, width=130, height=50)

        btn_imprimer = Button(fact_Menu_frame, command=self.imprimer_facture, text="Imprimer", font=("goudy old style",15, "bold"), cursor="hand2", bg="lightgreen").place(x=160, y=65, width=150, height=40)
        btn_effacer = Button(fact_Menu_frame, command=self.clear_all, text="Reinitialiser Tous", font=("goudy old style",15, "bold"), cursor="hand2", bg="lightgray").place(x=330, y=65, width=170, height=40)
        btn_generer = Button(fact_Menu_frame, command=self.generer_facture, text="Generer", font=("goudy old style",15, "bold"), cursor="hand2", bg="yellow").place(x=5, y=65, width=130, height=40)
    ...
    def generer_facture(self):
        if self.var_cli_nom.get()=="":
            ...
            self.body_facture()
            self.footer_facture()
            fp = open(fr"C:\Users\HP\Desktop\Python_Projects\gestion_magasin\facture\{str(self.facture)}.txt","w")
